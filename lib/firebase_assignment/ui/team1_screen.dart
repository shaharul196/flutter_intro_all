...
import 'package:ostad_flutter_sazu/firebase_assignment/data/liveScoreModel.dart';

class ArgentinaVSAfricaScreen extends StatefulWidget {
  const ArgentinaVSAfricaScreen({super.key});

  @override
  State<ArgentinaVSAfricaScreen> createState() =>
      _ArgentinaVSAfricaScreenState();
}

class _ArgentinaVSAfricaScreenState extends State<ArgentinaVSAfricaScreen> {
  ...
  Future<void> _getLiveScoreData() async {
    ...
    final QuerySnapshot<Map<String, dynamic>> snapshots =
        await db
            .collection('Football')
            .where('team1_name', isEqualTo: 'Argentina')
            .where('team2_name', isEqualTo: 'Africa')
            .get();
